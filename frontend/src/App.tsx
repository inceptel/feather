...
export default function App() {
  ...
  const tabStyle = (t: string) => ({
    ...
    background: 'none', color: tab() === t ? '#e5e5e5' : '#aeb6c2', 'font-size': '13px', 'font-weight': '600', cursor: 'pointer',
  ...
  return (
    <div
            ...
            <div style={{ flex: '1', 'overflow-y': 'auto', '-webkit-overflow-scrolling': 'touch', 'overscroll-behavior': 'contain', 'touch-action': 'pan-y', 'padding-bottom': 'max(12px, env(safe-area-inset-bottom, 0px))' }}>
              <For each={sessions()}>{(s) => (
                <button
                  ...
                  style={{ width: '100%', padding: '12px max(16px, env(safe-area-inset-right, 0px)) 12px max(16px, env(safe-area-inset-left, 0px))', 'min-height': '44px', cursor: 'pointer', border: 'none', 'border-left': s.id === currentId() ? '3px solid #4aba6a' : '3px solid transparent', background: s.id === currentId() ? '#1a1a2e' : 'transparent', 'border-bottom': '1px solid #111', color: 'inherit', 'text-align': 'left' }}
                >
                  <div style={{ display: 'flex', 'align-items': 'flex-start', gap: '8px' }}>
                    <Show when={s.isActive}><span style={{ width: '6px', height: '6px', 'border-radius': '50%', background: '#4aba6a', 'flex-shrink': '0', 'margin-top': '5px' }} /></Show>
                    <div style={{ flex: '1', 'min-width': '0', display: 'flex', 'flex-direction': 'column', gap: '4px' }}>
                      <span style={{ 'font-size': '13px', 'font-weight': '500', overflow: 'hidden', display: '-webkit-box', '-webkit-box-orient': 'vertical', '-webkit-line-clamp': '2', 'white-space': 'normal', 'overflow-wrap': 'anywhere', 'line-height': '1.25' }}>{formatSessionTitle(s.title)}</span>
                      <span style={{ 'font-size': '11px', color: '#aeb6c2', 'line-height': '1.2' }}>{timeAgo(s.updatedAt)}</span>
                    </div>
                  </div>
                </button>
              )}</For>
      ...
      <div aria-hidden={sidebar() ? 'true' : undefined} style={{ flex: '1', display: 'flex', 'flex-direction': 'column', 'min-width': '0', height: '100%', 'pointer-events': sidebar() ? 'none' : 'auto', overflow: 'hidden' }}>
        {/* Header */}
        <div style={{ padding: 'max(8px, env(safe-area-inset-top, 0px)) max(16px, env(safe-area-inset-right, 0px)) 8px calc(64px + env(safe-area-inset-left, 0px))', 'border-bottom': '1px solid #1e1e1e', display: 'flex', 'align-items': 'flex-start', gap: '6px', 'min-height': '48px', 'flex-shrink': '0' }}>
        ...
        <div style={{ flex: '1', overflow: 'hidden' }}>
          <Show when={currentId()} fallback={
            <div style={{ display: 'flex', 'align-items': 'center', 'justify-content': 'center', height: '100%', color: '#aeb6c2', padding: 'max(24px, env(safe-area-inset-top, 0px)) max(24px, env(safe-area-inset-right, 0px)) max(24px, env(safe-area-inset-bottom, 0px)) max(24px, env(safe-area-inset-left, 0px))' }}>
              <div style={{ 'text-align': 'center', width: '100%', 'max-width': '320px', display: 'flex', 'flex-direction': 'column', 'align-items': 'center', gap: '12px' }}>
                <p style={{ color: '#c9d1d9', 'font-size': '15px', 'font-weight': '500', 'line-height': '1.5', 'max-width': '22ch', margin: '0 auto' }}>Open a session or create a new one</p>
                <button
        ...
        <Show when={currentId() && tab() === 'chat'}>
          ...
          <div style={{ padding: '8px max(12px, env(safe-area-inset-right, 0px)) max(12px, env(safe-area-inset-bottom, 0px)) max(12px, env(safe-area-inset-left, 0px))', 'border-top': files().length ? 'none' : '1px solid #1e1e1e', background: '#0a0e14', display: 'flex', gap: '8px', 'align-items': 'flex-end', 'flex-shrink': '0' }}>
            <button
            ...
            <textarea ref={textareaRef} value={text()}
              ...
              style={{ flex: '1', 'min-width': '0', background: '#1a1a2e', border: '1px solid #333', 'border-radius': '12px', padding: '10px 14px', color: '#e5e5e5', 'font-size': '15px', 'font-family': 'inherit', resize: 'none', outline: 'none', 'line-height': '1.4', 'min-height': '44px', 'box-sizing': 'border-box', 'max-height': '180px' }} />
            <button aria-label="Send message" onClick={handleSend} disabled={uploading() || (!text().trim() && files().length === 0)} style={{ background: (text().trim() || files().length) ? '#4aba6a' : '#333', color: (text().trim() || files().length) ? '#000' : '#aeb6c2', border: 'none', 'border-radius': '12px', padding: '0 16px', 'font-size': '15px', 'font-weight': '600', cursor: (text().trim() || files().length) ? 'pointer' : 'default', 'min-height': '44px', 'min-width': '64px', display: 'inline-flex', 'align-items': 'center', 'justify-content': 'center', 'flex-shrink': '0' }}>{uploading() ? '...' : 'Send'}</button>
          </div>
        </Show>
      </div>
    </div>
