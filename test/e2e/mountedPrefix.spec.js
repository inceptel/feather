...
test.beforeAll(async () => {
  ...
  proxy = await startProxy([['/feather2', normalPort], ['/canary-preview', canaryPort]])
...
test('mobile read-only canary rejects terminal and shell through its mounted WS routes', async ({ page }) => {
  ...
  await page.goto(`${origin}/canary-preview/#${sessionId}`, { waitUntil: 'domcontentloaded' })
  await expect(page.getByText('Mounted prefix fixture', { exact: true }).first()).toBeVisible()
  await page.getByRole('button', { name: 'Terminal', exact: true }).click()
  await page.evaluate(() => Promise.all(['/api/terminal?session=mounted-prefix-session', '/api/shell'].map(route => new Promise(resolve => {
    ...
    const socket = new WebSocket(`${protocol}//${location.host}/canary-preview${route}`)
    socket.addEventListener('close', resolve)
    socket.addEventListener('error', resolve)
  }))))

  await expect.poll(() => proxyState.upgrades.filter(entry => entry.path.startsWith('/canary-preview/api/')).length).toBeGreaterThanOrEqual(2)
  expect(proxyState.upgrades.filter(entry => entry.path.startsWith('/canary-preview/api/')).every(entry => entry.status === 403)).toBe(true)
  expect(paths.some(value => value.startsWith('/canary-preview/assets/'))).toBe(true)
  expect(paths.some(value => value === `/canary-preview/api/sessions/${sessionId}/stream`)).toBe(true)
