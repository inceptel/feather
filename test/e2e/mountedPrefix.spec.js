...
test.beforeAll(async () => {
  ...
  proxy = await startProxy([['/feather2', normalPort], ['/canary-legacy-user', canaryPort]])
...
test('production prefix carries SPA assets, REST, SSE, media, files, export, WS, and reload', async ({ page }) => {
  ...
  const initialHealth = page.waitForResponse(response => new URL(response.url()).pathname === '/feather2/api/health')
  ...
  proxyState.versionOverride = 'mounted-prefix-new-version'
...
test('mobile read-only canary rejects terminal and shell through its mounted WS routes', async ({ page }) => {
  ...
  await page.goto(`${origin}/canary-legacy-user/#${sessionId}`, { waitUntil: 'domcontentloaded' })
  await expect(page.getByText('Mounted prefix fixture', { exact: true }).first()).toBeVisible()
  await page.getByRole('button', { name: 'Terminal', exact: true }).click()
  await page.evaluate(() => Promise.all(['/api/terminal?session=mounted-prefix-session', '/api/shell'].map(route => new Promise(resolve => {
    ...
    const socket = new WebSocket(`${protocol}//${location.host}/canary-legacy-user${route}`)
    socket.addEventListener('close', resolve)
    socket.addEventListener('error', resolve)
  }))))

  await expect.poll(() => proxyState.upgrades.filter(entry => entry.path.startsWith('/canary-legacy-user/api/')).length).toBeGreaterThanOrEqual(2)
  expect(proxyState.upgrades.filter(entry => entry.path.startsWith('/canary-legacy-user/api/')).every(entry => entry.status === 403)).toBe(true)
  expect(paths.some(value => value.startsWith('/canary-legacy-user/assets/'))).toBe(true)
  expect(paths.some(value => value === `/canary-legacy-user/api/sessions/${sessionId}/stream`)).toBe(true)
