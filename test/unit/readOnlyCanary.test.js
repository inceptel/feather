...
describe('server-enforced read-only canary', () => {
  it('allows the browsing surface while rejecting every mutation and terminal upgrade without state or tmux changes', async () => {
    ...
    const readable = [
      '/', '/api/health', '/api/boxes', '/api/sessions',
    ...
    const mutations = [
      ...
      ['POST', `/api/sessions/${fx.sessionId}/share`, { peers: ['viewer'] }],
      ...
      ['POST', '/api/rooms/test-room/residents', { role: 'caretaker', sessionId: fx.sessionId }],
      ['DELETE', '/api/rooms/test-room/residents/caretaker'],
  ...
  it('returns stable durable send receipts while preserving unkeyed client behavior', async () => {
    ...
    const sharedEndpoint = `${running.base}/api/share/sessions/${fx.sessionId}/send`
    const sharedHeaders = {
      Authorization: 'Bearer read-only-token',
      'Content-Type': 'application/json',
      'X-Feather-Message-ID': 'peer-voice-recovery-0001',
    }
    const sharedFirst = await fetch(sharedEndpoint, {
      method: 'POST', headers: sharedHeaders, body: JSON.stringify({ text: 'peer deliver once' }),
    })
    const sharedRetry = await fetch(sharedEndpoint, {
      method: 'POST', headers: sharedHeaders, body: JSON.stringify({ text: 'peer deliver once' }),
    })
  ...
  it('rejects the add-peer CLI mutation before changing state', () => {
    const fx = fixture()
    const before = inventory(fx.state)
    const result = spawnSync(process.execPath, ['server.js', '--add-peer', 'blocked-peer'], {
      cwd: REPO,
      env: { ...process.env, HOME: fx.home, FEATHER_STATE_DIR: fx.state, FEATHER_READ_ONLY: '1' },
      encoding: 'utf8',
    })
    assert.equal(result.status, 1)
    assert.match(result.stderr, /cannot add a peer while FEATHER_READ_ONLY is enabled/)
    assert.deepEqual(inventory(fx.state), before)
  })
})
