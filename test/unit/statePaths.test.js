...
describe('state path classification', () => {
  ...
  it('moves only instance state under the configured root', () => {
    ...
    assert.equal(paths.coordination.roomMainsFile, '/home/zak/.feather/room-mains.json')
...
describe('server state integration', () => {
  it('reads and writes sharing state only in FEATHER_STATE_DIR', () => {
    const base = tempDir('feather-server-state-')
    const stateDir = path.join(base, 'state')
    const homeDir = path.join(base, 'home')
    fs.mkdirSync(stateDir)
    fs.mkdirSync(homeDir)
    fs.writeFileSync(path.join(stateDir, 'sharing.json'), JSON.stringify({ owner: 'zak' }), { mode: 0o644 })
    ...
    assert.equal(sharing.owner, 'zak', 'existing copied state must remain readable')
