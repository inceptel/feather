...
describe('Room autonomy prompts', () => {
  ...
  it('primes the builder and checker halves of an agent, and the house helpers', () => {
    ...
    const focused = builderWakePrompt({ roomName: 'ev-shop', agentName: 'research', group: 'g2', at, budgetMs: 1_800_000, roundMs: 480_000, checkerEngine: 'codex', focus: 'Scan primary sources since the last approved brief.' })
    assert.ok(focused.includes('## Scheduled focus\n\nScan primary sources since the last approved brief.'))
    assert.ok(focused.includes('Apply this focus before the generic queue instructions'))
