...
export function roomTemplateFiles({ name, mission, now = new Date() }) {
  ...
  const updater = [
    ...
    '',
    'Cadence: at most one card per wake, and normally not more than two or three per day for a Room that is working well. If two candidates compete, publish the one the user can act on. An empty wake is correct and common.',
    'Each item needs a stable evidence id (a wiki page plus date, a note marker such as `notes.md#<timestamp>-plan`, a session id) and publishes once.',
    '',
    '## A wake, in order',
    '',
    ...
    '4. The marketer returns a publication JSON path. Read it, verify every claim against the evidence, and correct anything wrong. The `id` and `visual` must be plain slugs (letters, digits, `.`, `_`, `-`); `sourceEvidenceId` stays the evidence id verbatim.',
  ...
  const marketer = [
    ...
    `3. Make the image with Gemini: \`room visual --out ${roomPath}/artifacts/<slug>.png "<image prompt>"\`. Describe a clean, specific illustration of the update (no text baked into the image, no logos, no faces). \`room visual\` falls back to a rendered text card if Gemini is unavailable and prints which path it used.`,
    '4. Check the PNG exists and is under 5 MiB. Write factual alt text.',
    `5. Write \`${roomPath}/artifacts/<slug>.json\` (keep \`sourceEvidenceId\` as the evidence id, verbatim):`,
    '   `{"id": "<slug>", "sourceEvidenceId": "<evidence-id>", "title": "...", "summary": "...", "detail": "...", "visual": "artifacts/<slug>.png", "visualAlt": "..."}`',
