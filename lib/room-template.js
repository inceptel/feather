...
export function roomTemplateFiles({ name, mission, now = new Date() }) {
  ...
  const updater = [
    ...
    `4. For a By-the-way item that needs no visual, write \`${roomPath}/.updater/<slug>.json\` yourself. Otherwise read the marketer's returned JSON. Verify every claim. The \`id\` and \`visual\` must be plain slugs (letters, digits, \`.\`, \`_\`, \`-\`); \`sourceEvidenceId\` stays verbatim.`,
  ...
  const marketer = [
    ...
    '## For each brief',
    '',
    '1. Read the brief and cited evidence (Wiki page, note). Preserve its `briefing` or `by-the-way` attention level. Write the card text: title (≤80 chars), summary (≤500 chars, the point in plain words), detail (optional, ≤1000 chars).',
