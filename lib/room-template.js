...
export function leaderWakePrompt({ roomName, at = new Date() }) {
  return [
    ...
    'This wake has a fixed budget: about one hour, then the chat is retired whether or not it finished. Keep every tool call under five minutes; bound a long scan to a sample or a block range and say so. Write the `room note` and move the line by the 45-minute mark even if the result is partial, so the next wake continues instead of restarting.',
