...
export function leaderWakePrompt({ roomName, at = new Date() }) {
  return [
    ...
    'Pick the most valuable open gap in FRONTIER.md and move it to Working. If Open is empty and the mission is not done, add the gaps you can see first (one line each, why it matters), then pick one.',
    ...
    'If the mission is materially done and Open is empty, say so plainly and stop. Do not invent work.',
  ].join('\n')
}
...
export function judgeWakePrompt({ roomName, leaderSessionId, leaderWakeAt, at = new Date() }) {
  return [
    ...
    'For every line under Review, and every line under Working older than that wake, open the real artifact it cites and grade it against the mission. Move each line to Done or back to Open with a dated verdict, as JUDGE.md says.',
...
export function roomTemplateFiles({ name, mission, now = new Date() }) {
  ...
  const judge = [
    ...
    '   - Done: `- <line> ✓ judged YYYY-MM-DD: <one clause on what makes it done>`',
    '   - Open: `- <line> ↩ judged YYYY-MM-DD: <exactly what is missing, in one line the Leader can act on>`',
    ...
    '- FRONTIER.md is the only file you change, and only Review → Done, Review → Open, Working → Open, and additions under Open.',
...
export function frontierTemplate(name) {
  return [
    ...
    '## Steering',
    '',
    '<!-- The user writes here: priorities, constraints, what "done" means. Agents read, never edit. -->',
    '',
    '## Open',
    '',
