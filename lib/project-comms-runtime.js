...
export function projectCommsSources(meta, inbox, wikiPath) {
  ...
  for (const [sessionId, chat] of Object.entries(meta)) {
    if (chat.chatRole !== 'creator' || !chat.chatProjectId || !chat.cwd) continue
...
export function createProjectCommsRuntime({ store, root, wikiPath, readMeta, inbox, launch, sendTask, baseUrl, refresh = () => {}, readOnly = false, dispatchEnabled = true }) {
  ...
  let ticking = false, lastError = null
  ...
  async function tick() {
    ...
    try {
      ...
      for (const delegation of store.pendingDelegations()) {
        ...
        if (!meta || meta.chatProjectId !== delegation.projectId || meta.chatRole !== 'creator') throw new Error('Comment owner is no longer available')
