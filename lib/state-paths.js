...
const INSTANCE_NAMES = Object.freeze({
  ...
  feedPreferencesFile: 'feed-preferences.json',
  feedCommentsFile: 'feed-comments.json',
  providerLimitsFile: 'provider-limits.json',
})
...
export function resolveStatePaths({
  ...
  return {
    ...
    instance,
    // Derived, disposable data rebuilt from the harness transcripts. Deleting
    // this directory loses nothing; read-only checks ignore it.
    cache: {
      root: path.join(instanceRoot, 'cache'),
      searchIndexFile: path.join(instanceRoot, 'cache', 'session-search.db'),
      sessionFactsFile: path.join(instanceRoot, 'cache', 'session-facts.json'),
    },
    coordination: {
      ...
      roomLeaderWakesFile: path.join(featherHome, 'room-leader-wakes.json'),
      roomPulsesFile: path.join(featherHome, 'room-pulses.json'),
      schedulerFile: path.join(featherHome, 'scheduler.json'),
      schedulerRunsFile: path.join(featherHome, 'scheduler-runs.jsonl'),
    ...
    harness: {
      ...
      ompAgentDirsDir: path.join(featherHome, 'omp-agents'),
