...
import { wikiPageFromEvidenceId } from './room-frontier.js'
...
const MAX_PUBLICATION_SUMMARY_CODE_POINTS = 9_000
const MAX_PUBLICATION_DETAIL_CODE_POINTS = 9_000
...
export function buildSuperFeed({ rooms = [], complaints = [], publications = [] } = {}) {
  ...
  for (const room of rooms) {
    // Leader chat is not a feed source. Cards come from Updater publications,
    // #friction records, and active Room status failures only.
  ...
  for (const publication of publications) {
    ...
    const attention = publication.attention === 'by-the-way' ? 'by-the-way' : 'briefing'
    items.push({
      ...
      sourceState: 'available',
      attention,
      status: attention === 'by-the-way' ? 'by the way' : 'briefing',
      needsReview: false,
      sessionId: null,
      publicationId: publication.id,
      wikiPage: wikiPageFromEvidenceId(publication.sourceEvidenceId),
