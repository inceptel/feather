...
function waitingBoundary(text) {
  const match = String(text || '').match(/(?:^|\n)RALPH_WAITING:\s*(.+?)\s*$/i)
  return match ? { waiting: match[1].slice(0, 1_000) } : {}
}
...
export function ralphBoundaryFromLine(line, agent) {
  ...
  if (agent === 'omp') {
    ...
    return { type: 'completed', key: keyFor(entry, entry.timestamp), blocked: blockedReason(text), ...waitingBoundary(text), ...(complete ? { complete } : {}) }
  }
