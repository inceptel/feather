...
export const ROOM_PUBLICATION_MIN_INTERVAL_MS = 30 * 60 * 1000
export const ROOM_PUBLICATION_DAILY_LIMIT = 3
...
function normalizedInput(input) {
  ...
  return {
    ...
    summary: text(input.summary, 600, 'summary', { required: true }),
    detail: text(input.detail, 1_200, 'detail'),
    visual,
    visualAlt,
  }
}
...
function samePublication(left, right) {
  return ['id', 'sourceEvidenceId', 'title', 'summary', 'detail', 'visual', 'visualAlt']
...
export function appendRoomPublication({ roomRoot, roomName, publisherSessionId, input, now = new Date() }) {
  ...
  if (byId) {
    if (!samePublication(byId, normalized)) throw publicationError(409, 'publication id already exists with different content')
    return { record: byId, reused: true }
  }
  ...
  const today = existing.filter(record => record.occurredAt.slice(0, 10) === day)
  if (today.length >= ROOM_PUBLICATION_DAILY_LIMIT) throw publicationError(429, 'Room publication daily limit reached')
  const latestMs = existing.reduce((latest, record) => Math.max(latest, Date.parse(record.occurredAt)), 0)
  if (latestMs && now.getTime() - latestMs < ROOM_PUBLICATION_MIN_INTERVAL_MS) {
    throw publicationError(429, 'Room publications must be at least 30 minutes apart')
  }
