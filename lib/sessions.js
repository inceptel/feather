...
const ACTIVITY_CHUNK_BYTES = 64 * 1024
const ACTIVITY_MAX_BYTES = 32 * 1024 * 1024

function messageLineTimestampMs(line, agent) {
  if (!line.length) return null
  try { return messageTimestampMs(JSON.parse(line.toString('utf8')), agent) }
  catch { return null }
}

// Find the newest real message without repeatedly decoding overlapping,
// multi-megabyte tails. Most transcripts resolve in the first 64KB; older
// messages are reached one bounded chunk at a time.
export function lastMessageMsFromFile(fpath, agent, fallbackMs = 0, {
  chunkBytes = ACTIVITY_CHUNK_BYTES,
  ...
  try {
    ...
    try {
      let position = size
      let remaining = Math.min(size, Math.max(1, maxBytes))
      let suffix = Buffer.alloc(0)
      while (position > 0 && remaining > 0) {
        const length = Math.min(Math.max(1, chunkBytes), position, remaining)
        position -= length
        remaining -= length
        ...
        while (end > 0) {
          ...
          if (newline < 0) break
          const timestamp = messageLineTimestampMs(data.subarray(newline + 1, end), agent)
      ...
      if (position === 0) {
        const timestamp = messageLineTimestampMs(suffix, agent)
        if (timestamp) return timestamp
      }
    } finally {
      fs.closeSync(fd)
