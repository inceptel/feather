...
export const ENGINES = Object.freeze(['claude', 'codex', 'omp'])
export const MODES = Object.freeze(['inject', 'fresh'])
export const TARGET_KINDS = Object.freeze(['leader', 'resident', 'session', 'new', 'agent'])
// Reserved rule namespace for chat-owned schedules (no Room directory).
export const CHAT_RULES_ROOM = 'chats'
export const CHAT_OWNER_RE = /^[A-Za-z0-9-]{8,80}$/
export const CRITERIA = Object.freeze(['idle', 'file-changed', 'file-matches', 'frontier-has', 'todo-has', 'wiki-approved'])
export const DEFAULT_ROUND_MS = 10 * 60_000
export const RUN_OUTCOMES = Object.freeze(['done', 'failed', 'timeout', 'killed', 'stopped'])
...
function normalizeTarget(input) {
  ...
  if (kind === 'agent') {
    // One agent = a builder chat plus a checker chat, each on any harness.
    const party = (spec, label) => {
      const engine = spec?.engine || 'claude'
      if (!ENGINES.includes(engine)) fail(`target.${label}.engine must be one of ${ENGINES.join(', ')}`)
      return { engine, model: optionalText(spec?.model, `target.${label}.model`, 200) }
    }
    const roundMs = input.roundMs === undefined || input.roundMs === null ? DEFAULT_ROUND_MS : parseDuration(input.roundMs)
    if (!roundMs) fail('target.roundMs must be a duration')
    return { kind, builder: party(input.builder, 'builder'), checker: party(input.checker || { engine: 'codex' }, 'checker'), roundMs }
  }
...
function normalizeCriterion(input) {
  ...
  if (type === 'frontier-has' || type === 'todo-has') {
    const section = String(input.section || (type === 'todo-has' ? 'Open' : 'Review'))
    if (!/^[A-Za-z][A-Za-z ]{0,31}$/.test(section)) fail('when.section must be a section name')
    return { type, section }
  }
  if (type === 'wiki-approved') return { type }
...
export function normalizeRule(input) {
  ...
  const mode = input.mode ?? (target.kind === 'new' || target.kind === 'agent' ? 'fresh' : 'inject')
  if (!MODES.includes(mode)) fail(`mode must be one of ${MODES.join(', ')}`)
  if (mode === 'fresh' && !['new', 'leader', 'agent'].includes(target.kind)) fail('mode fresh needs target.kind new, agent, or leader')
  if (mode === 'inject' && (target.kind === 'new' || target.kind === 'agent')) fail(`target.kind ${target.kind} needs mode fresh`)
  ...
  let ownerSessionId
  if (room === CHAT_RULES_ROOM) {
    // A chat rule only ever injects into the chat that owns it.
    if (target.kind !== 'session' || mode !== 'inject') fail(`${CHAT_RULES_ROOM} rules need target.kind session and mode inject`)
    ownerSessionId = input.ownerSessionId === undefined || input.ownerSessionId === null ? target.sessionId : String(input.ownerSessionId)
    if (!CHAT_OWNER_RE.test(ownerSessionId)) fail('ownerSessionId must be a session id')
    if (ownerSessionId !== target.sessionId) fail('ownerSessionId must equal target.sessionId')
  } else if (input.ownerSessionId !== undefined && input.ownerSessionId !== null) {
    fail(`ownerSessionId is only valid for ${CHAT_RULES_ROOM} rules`)
  }
  return {
...
export function emptyRuntime() {
  return {
    lastRunAt: null, previousRunAt: null, lastRunId: null, lastOutcome: null, nextDueAt: null,
...
export function checkCriterion(criterion, rule, runtime, context) {
  ...
  if (criterion.type === 'todo-has') {
    const text = context.fileText?.('wiki/TODO.md')
    if (typeof text !== 'string') return 'wiki/TODO.md missing'
    return frontierSectionHasLines(text, criterion.section) ? true : `TODO ${criterion.section} is empty`
  }
  if (criterion.type === 'wiki-approved') {
    // Any Room's wiki/Log.md written since this rule last ran (global helpers).
    const since = Date.parse(runtime.lastRunAt || '') || 0
    const rooms = context.wikiWrittenSince?.(since)
    if (!Array.isArray(rooms)) return 'no wiki activity data'
    return rooms.length ? true : 'no approved wiki writes since last run'
  }
...
export function markStarted(runtime, { runId, at, parentRunId = null }) {
  ...
  return {
    ...runtime,
    // Wake prompts ask "what changed since the run before this one?"; keep it.
    previousRunAt: runtime.lastRunAt || null,
    lastRunAt: startedAt,
    lastRunId: runId,
    lastOutcome: null,
...
export function describeRule(rule, runtime, activeRun, { now, bootAt }) {
  ...
  return {
    ...
    runtime: {
      ...
      running: activeRun ? { runId: activeRun.runId, startedAt: activeRun.startedAt, sessionId: activeRun.sessionId || null, ...(activeRun.agent ? { agent: { groupId: activeRun.agent.groupId, checkerSessionId: activeRun.agent.checkerSessionId, roundMs: activeRun.agent.roundMs } } : {}) } : null,
    },
  }
}
