...
export function appendSteering(frontierText, text, at = new Date(), { heading = 'Steering' } = {}) {
  ...
  let start = rows.findIndex((row) => headingRe.test(row))
  if (start === -1) {
    const title = rows.findIndex((row) => /^#\s/.test(row))
    const insertAt = title === -1 ? 0 : title + 1
    rows.splice(insertAt, 0, '', `## ${heading}`, '')
    start = insertAt + 1
  }
  let end = rows.length
