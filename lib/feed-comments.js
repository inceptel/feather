...
export function feedCommentPrompt({ commentId, roomName, item, text }) {
  const lines = [
    ...
    text,
    '',
    'Reply FIRST, within a minute, with what you already know; do not research before replying. Only this command shows under the card (chat text, notes, and wiki edits do not):',
    `  room reply ${commentId} --stdin <<'REPLY'`,
    '  <your answer, standing on its own>',
    '  REPLY',
    'If a full answer needs more work, say so in that first reply, do the work, then run the same command again; the newer reply replaces the older one.',
  ]
  return lines.join('\n')
}

// Sent once when a comment still has no stored reply after a while: the
// Leader usually read it and wandered into research (seen 2026-09-06).
export function feedReplyNudgePrompt({ commentId, roomName, text }) {
  return [
    `${FEED_COMMENT_PREFIX} reminder · #${roomName}] ${marker(commentId)}`,
    `The user is still waiting under the card for an answer to: ${text.slice(0, 200)}`,
    'Stop what you are doing and reply now with what you have, even if partial:',
    `  room reply ${commentId} --stdin <<'REPLY'`,
    '  <your answer>',
    '  REPLY',
    'Then continue.',
...
// The house replyguy works a queue file, not a chat inbox: one line per
// comment, flipped from `- open` to `- done` once the reply is stored. The
// line carries everything a fresh chat needs to answer without the server.
export function feedCommentQueueLine({ commentId, roomName, item, text, at = new Date() }) {
  const oneLine = (value, max) => String(value || '').replace(/\s+/g, ' ').trim().slice(0, max)
  const stamp = at.toISOString().slice(0, 16).replace('T', ' ')
  return `- open ${commentId} #${roomName} ${stamp} card="${oneLine(item?.title, 160).replaceAll('"', "'")}" evidence=${oneLine(item?.evidenceId, 200) || '-'} :: ${oneLine(text, FEED_COMMENT_MAX_CHARS)}`
}
