...
const GITHUB_SKILL_PATH_RE = /github\.com\/[^/]+\/(?:[^/?#]*skill[^/?#]*|council)(?:[/?#]|$)/iu
const CAPABILITY_AUTHORING_VERB_RE = /\b(?:build|create|make|write|generate|install|add)\b/iu
const NAMED_TARGET_RE = /\bnamed\s+(?:`[a-z0-9][a-z0-9._-]{2,79}`|"[a-z0-9][a-z0-9._-]{2,79}"|'[a-z0-9][a-z0-9._-]{2,79}'|[a-z0-9][a-z0-9._-]{2,79})|(?:`[a-z0-9][a-z0-9._-]{2,79}`|"[a-z0-9][a-z0-9._-]{2,79}"|'[a-z0-9][a-z0-9._-]{2,79}')/iu
...
function hasImperativeNamedSkill(text) {
  const source = String(text)
  for (const match of source.matchAll(/\bskill\b/igu)) {
    const start = Math.max(0, match.index - 120)
    const window = source.slice(start, match.index + match[0].length + 180)
    if (CAPABILITY_AUTHORING_VERB_RE.test(window) && NAMED_TARGET_RE.test(window)) return true
  }
  return false
}
...
export function classifyBookmark(bookmark, { roomExists = () => true, skillCollision = false } = {}) {
  ...
  const githubSkillPath = bookmark.canonicalUrls.some((url) => GITHUB_SKILL_PATH_RE.test(url))
  const skillCandidate = githubSkillPath
    || (/\bskill\b/iu.test(haystack) && Boolean(githubRepository || hasImperativeNamedSkill(bookmark.text)))
  let room = 'feather'
