...
export function createProviderLimits({
  ...
  const state = {
    anthropic: { fetchedAt: 0, data: null, error: null, lastGoodAt: null },
    openrouter: { fetchedAt: 0, data: null, error: null, lastGoodAt: null },
  }
  ...
  async function refreshAnthropic() {
    ...
    try {
      ...
      slot.error = error.status === 429
        ? (slot.data ? 'Anthropic is rate-limiting its usage endpoint; showing the last good reading' : 'Anthropic is rate-limiting its usage endpoint; retrying every 5 minutes')
        : error.status === 403 ? 'Anthropic token lacks the user:profile scope'
  ...
  function refreshDue(name, refresh) {
    ...
    if (now() - slot.fetchedAt < pollMs) return inFlight.get(name) || Promise.resolve()
    slot.fetchedAt = now()
