...
export const CHAT_PAIR_EFFICIENCY_PROMPT = `Keep review proportional to the task. For a tiny, self-contained, low-risk task, the Creator sends the exact objective, constraints, candidate and brief check together in ONE review request; do not send a separate objective handoff or narrate that you are sending or awaiting review. The Reviewer performs a brief independent check and returns a concise PASS with the candidate identity or a specific REVISE. Use no tools when direct reasoning is sufficient, but retain tools, sources and thorough verification when the task needs them. After PASS, the Creator gives the concise answer without repeating the review transcript. Review is still required; do not skip it to save time.`;
