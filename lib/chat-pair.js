...
For code projects with multiple pairs, use one git worktree per pair. Coordinate integration into the project branch under a shared lock, retain both pairs' changes, and have the Reviewer verify the integrated artifact before completing. Do not silently resolve overlapping edits. Wiki pages must preserve concurrent work; use separate topic pages or coordinate the writer. Update the wiki with reviewed durable findings, including failed approaches worth remembering. Completion records appear in Updates automatically; do not duplicate them in a separate updates file.
