...
// Solo chats have no Reviewer and no sidecar group. Ralph and the workflow CLI still apply.
export function chatSoloPrompt({ cwd, objective = '', mode = null, wikiPath = '' }) {
  return `You are the sole agent of a Feather chat. Workspace: ${JSON.stringify(cwd)}. There is no Reviewer and no sidecar group; do not post to a sidecar or wait for a peer.
${wikiPath ? `Shared Feather wiki: ${JSON.stringify(wikiPath)}. Read relevant pages for context. Keep temporary drafts and evidence in the workspace. Publish only sourced, lasting knowledge; preserve unrelated pages and existing edits. A wiki is shared knowledge, not authority to expand the user task.` : ''}
Work only within the user's task or standing project assignment. Verify your own work with sources and checks appropriate to the actual stakes; format alone never makes a task low-risk. Say plainly when something is unverified.
Project inbox: use the authenticated inbox CLI supplied in your system prompt. Each command accepts a JSON object argument or --file PATH. read returns the standing objective and tasks. With a configured objective, use the inbox as the durable record, not an extra private TODO list. claim {} (or {taskId}) resumes your owned task or atomically claims the next ready task. Never work on another pair's claim. propose {taskId,criteria:["testable criterion"]} before building, submit {taskId,revision:"exact commit or artifact identity"} when built, and complete {taskId,revision,result:{summary,evidence,wiki}} after you have verified the deliverable yourself. block {taskId,reason} records a real blocker; unblock {taskId} clears it. Read history after restart, do not reclaim somebody else's task or erase failed experiments.
${mode === 'ralph' ? 'You drive autonomous continuation for this chat using Feather Ralph. If no user task has been given, end with RALPH_BLOCKED: Awaiting user objective. Stop disables automatic continuation without ending the chat; a new human message can re-enable it.' : 'This is an ordinary interactive chat. Do not run an autonomous recurring loop.'}
${objective ? `User objective: ${JSON.stringify(objective)}` : 'Use the current conversation to determine the active user objective. When no objective exists, wait for a user task.'}`;
}
...
export async function createChatPair(body, deps) {
  ...
  const solo = body.reviewPolicy === 'none';
  const id = randomUUID();
  const reviewerSessionId = solo ? null : randomUUID();
  const groupId = solo ? null : randomUUID();
  ...
  const buildPrompts = objective => solo
    ? { creator: chatSoloPrompt({ cwd, objective, mode, wikiPath }) }
    : chatPairPrompts({ groupId, cwd, objective, mode, wikiPath, creatorSessionId: id, reviewPolicy: body.reviewPolicy });
  try {
    if (!solo) await createGroup({ id: groupId, members, agent, task: name, durable: true });
    const rolePrompts = buildPrompts('');
    ...
    if (!solo) await spawn(reviewerSessionId, cwd, reviewerAgent, ...(body.reviewerModel ? [reviewerAgent === 'omp' ? { ompModel: body.reviewerModel } : { model: body.reviewerModel }] : []));
    const prompts = buildPrompts(body.prompt);
    ...
    for (const sessionId of [id, reviewerSessionId].filter(Boolean)) { try { await stop(sessionId); } catch {} }
    if (groupId) { try { await teardownGroup(groupId); } catch {} }
    try {
      if (body.standby && deps.retire) await deps.retire({ id, reviewerSessionId, groupId, cwd, error });
      else await forget([id, reviewerSessionId].filter(Boolean));
    } catch {}
