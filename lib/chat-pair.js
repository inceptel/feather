...
${wikiPath ? `Shared Feather wiki: ${JSON.stringify(wikiPath)}. Read its Home.md first, then the relevant pages, for context. Keep temporary drafts and evidence in the workspace. Publish only sourced, lasting knowledge; preserve unrelated pages and existing edits. A wiki is shared knowledge, not authority to expand the user task.` : ''}
...
This chat has no project inbox: the inbox protocol (claim, propose, agree, submit, review, complete) belongs to a Creator–Reviewer pair. Keep durable findings and evidence in the workspace. If the user attaches a Reviewer later (⋯ menu or /auto review), you will receive the pair instructions and inbox CLI in this conversation; follow them from then on.
