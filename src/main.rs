...
async fn tail_session(
    ...
    // Track sent message content by UUID — on file rewrite, only emit new/changed lines
            ...
            match read_from_offset(&file_path, current_offset) {
                Ok((lines, new_offset)) => {
                    ...
                    if lines.is_empty() {
                        ...
                        // Build events, deduplicating by UUID+content to avoid
                        // re-sending unchanged messages on file rewrite (SSE resync).
                        // Only truly new or updated messages are emitted.
                        ...
                        for line in &lines {
                            ...
                            // Skip if we already sent this exact line (same UUID, same content)
                            if let Some(uuid_str) = parsed.get("uuid").and_then(|v| v.as_str()) {
                                if let Some(prev_line) = sent_lines.get(uuid_str) {
                                    if prev_line == line {
                                        continue; // Unchanged — skip
                                    }
                                }
                                sent_lines.insert(uuid_str.to_string(), line.clone());
                            }
