...
impl TmuxManager {
    ...
    /// For long messages, writes to a temp file and uses `load-buffer`/`paste-buffer`
    /// to bypass the OS command-line argument length limit.
    pub fn send_message(&self, session_id: &str, message: &str) -> Result<(), String> {
        ...
        // For long messages, use load-buffer/paste-buffer to avoid arg length limits
        if message.len() >= 500 {
            self.send_message_via_buffer(&name, message)?;
        } else {
    ...
    /// Send a long message via tmux load-buffer/paste-buffer.
    ///
    /// Writes the message to a temp file, loads it into a tmux buffer,
    /// then pastes it into the target pane. This bypasses the OS limit
    /// on command-line argument length that affects `send-keys`.
    fn send_message_via_buffer(&self, tmux_name: &str, message: &str) -> Result<(), String> {
        use std::io::Write;

        // Write message to a temp file
        let tmp_path = format!("/tmp/feather-tmux-buf-{}", std::process::id());
        ...
        let paste = Command::new("tmux")
            .args(["paste-buffer", "-t", tmux_name, "-p"])
            .output()
