...
fn parse_jsonl_file(
    ...
    // Trim incomplete last line: if the file doesn't end with a newline,
    // Claude Code is likely mid-write. Drop the partial line to avoid parse errors.
    let content = if !file_content.ends_with('\n') && !file_content.is_empty() {
        match file_content.rfind('\n') {
            Some(pos) => {
                debug!("Trimming incomplete last line from {} ({} bytes after last newline)",
                    path.display(), file_content.len() - pos - 1);
                &file_content[..=pos]
            }
            None => {
                // Entire file is one incomplete line
                debug!("Skipping single incomplete line in {}", path.display());
                return Ok(true);
            }
        }
    } else {
    ...
    // Track if any lines failed to parse (for caller retry logic)
    ...
    for (idx, line) in lines.iter().enumerate() {
        let record: serde_json::Value = match serde_json::from_str(line) {
            ...
            Err(e) => {
                // Log mid-file errors for debugging (truncate line to 120 chars)
                let preview: String = line.chars().take(120).collect();
                debug!("JSONL parse error in {} line {}: {} — {:?}", path.display(), idx + 1, e, preview);
